...
def main(f_directed, n_gpu):  # f_directed: 0: undirected, 1: directed
    ...
    B = np.array([0.9, 0.1])
    S = 0.03
    p_outlier_list = np.arange(0.01, 0.11, 0.01)  # Ratio of outliers
    lr = 1e-2
    n_epoch = 200
    ...
    if f_directed == 0:  # undirected
        ...
        str_directed = 'u_outlier'
    else:  # directed
        n_units_in = np.array([2 * n, 10, 1])
        n_units_out = np.array([2, 10, 1])
        str_directed = 'd_outlier'
    clr_matrix = 'CMRmap_r'
    # ---------------------------------------------------------------------------------------------
    ...
    err = np.zeros((p_outlier_list.shape[0], n_replicate))
    err_pca = np.zeros((p_outlier_list.shape[0], n_replicate))
    err_svd = np.zeros((p_outlier_list.shape[0], n_replicate))
    err_mds = np.zeros((p_outlier_list.shape[0], n_replicate))
    ...
    for k in range(p_outlier_list.shape[0]):
        print('[sigma = {:.2f}'.format(p_outlier_list[k]) + ']')
        for m in range(n_replicate):
            A_bar, P_bar = gm.gen_matrix3(n, B, S, p_outlier_list[k], 1, f_directed, n_replicate * k + m + 1)
        ...
        pr.plot_compare_mean_std(p_outlier_list, err, err_pca, err_svd, err_mds, False, str_directed)
        pr.plot_compare_mean_std(p_outlier_list, err, err_pca, err_svd, err_mds, True, str_directed)
        pr.plot_compare_scatter(p_outlier_list, err, err_pca, err_svd, err_mds, False, str_directed)
        pr.plot_compare_scatter(p_outlier_list, err, err_pca, err_svd, err_mds, True, str_directed)
    plt.close('all')

    main.S_list = p_outlier_list
    main.err = err
    main.err_pca = err_pca
    main.err_svd = err_svd
    main.err_mds = err_mds
    dill.dump_session('result/outlier_' + str_directed + '.pkl')
